# Ported verbatim from reference/Anaconda/mmfparser/data/chunkloaders/actions/names.py
# Anaconda is (c) Mathias Kaerlev 2012, GPLv3.
"""Action code → human-name lookup tables.

Source: `mmfparser/data/chunkloaders/actions/names.py`.

- `ACTION_SYSTEM_NAMES` mirrors Anaconda's `systemDict` — nested
  `{object_type_id: {code: name}}`. Covers both built-in object types
  (negative IDs) and the 2-9 per-type slots (Active=2, Text=3, Question=4,
  Counter=7, RTF=8, SubApplication=9).
- `ACTION_EXTENSION_NAMES` mirrors Anaconda's flat `extensionDict` —
  the generic object-extension actions (SetPosition, SetX, Stop, Start,
  animation/direction control, alterable values, flags, layer ops,
  rigidbody ops, ...).
"""

from __future__ import annotations

# Nested map: object_type_id -> action_code -> name
ACTION_SYSTEM_NAMES: dict[int, dict[int, str]] = {
    2: {
        80: "PasteActive",
...
# Flat map: action_code -> name, for generic object-extension actions.
ACTION_EXTENSION_NAMES: dict[int, str] = {
    1: "SetPosition",
    2: "SetX",
    3: "SetY",
    4: "Stop",
